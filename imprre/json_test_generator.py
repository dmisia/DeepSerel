...
file_out = os.path.join('./data/retacred', "train_triples.json")
chunks = []
            ...
            for token in d:
                if len(token['relations']) >= 2:
                    ...
                    if token_annotation_type == 'spatial_indicator':
                        ...
                        # znajdź tokeny nie będące w żadnej relacji w tym zdaniu
                        non_rel_tokens = [e for e in d if not e['relations']]
                        # bierzemy trójki z całego zdania
                        list_of_candidates = [d[i:i + 3]  for i in range(len(d) - 2)]

                        # for triplet in itertools.permutations(list_of_candidates)]=
